...
description: 长对话帽身份丢失时先读本片段 re-anchor（hat_id / task_slug / 禁区）。当上下文脏、多轮读码后纪律漂移、或换帽后仍自称旧帽时使用。不用于：替代 prompts 全文；替代 npx spec-wave verify。
license: MIT
...
只做本 `hat_id` 允许的事。换帽须加载该帽 prompts **全文**。机械闸：`npx spec-wave verify`。
