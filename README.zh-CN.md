...
**现行包**：**`spec-wave@3.0.0`** — **已发布 2026-09-18**（人执行 publish · tag `v3.0.0` ↔ `895b975` · registry `latest=3.0.0`）。前一已发：**`2.4.2`**（验收修复 patch）· **`2.4.1`**（验收修复 patch）· **`2.4.0`**（门禁强度补全）· **`2.3.1`**（验收修复 patch）· **`2.3.0`**（接线补全）· **`2.2.1`**（验收修复 patch）· **`2.2.0`**（闭环起步）。
