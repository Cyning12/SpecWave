...
**现行包**：**`spec-wave@2.4.2`** — **bump 已落 · 待发版**（tag/push/publish 仅人；当前已 published=**`2.4.1`**（`latest` · tag `v2.4.1`））。前一已发：**`2.4.0`**（门禁强度补全）· **`2.3.1`**（验收修复 patch）· **`2.3.0`**（接线补全）· **`2.2.1`**（验收修复 patch）· **`2.2.0`**（闭环起步）。
