...
**SpecWave**（`spec-wave@2.3.0`）是 **多宿主编码 CLI**——单一声明式适配表原生落点 13 个宿主（Cursor · Claude Code · 可选 DSH · agents · Copilot · Codex · Windsurf · Gemini CLI · opencode · Roo Code · Zed · Cline · aider）——带 **P0 闸 / Harness 过程命令** 与 IDE 物化。曾用名 **SpecGate** / **dsh-coding-kit**。纪律资产仍是 ICVO（Inform · Constrain · Verify · Orchestrate）。
...
主入口是 npm 包 **`spec-wave@2.3.0`** 的 **`npx spec-wave`**。过渡 bin `specgate` / `dsh-coding-kit` 仍可用。插件面与 CLI 面互不替代。
...
钉 **spec-wave@2.3.0** 后可去掉 `@cyning/harness`。最小路径三步（必须，按序）：

1. 把 `devDependency` `@cyning/harness` 换成 `spec-wave`（钉 `2.3.0`；曾用名 `dsh-coding-kit`）。
2. 在仓根执行 `npx spec-wave upgrade --yes`（读优先 `.coding-kit/manifest.json`，否则 legacy `.cyning-harness/manifest.json`；**写入** `.coding-kit/manifest.json`，`version` 钉 2.3.0，`from_version` 记旧号；**不删除** `.cyning-harness/`）。
...
Skill 安装为 **推荐、非必须**（最小路径不依赖 DSH 扫 skill）。命令一律 `npx spec-wave`。旧包 **`@cyning/harness` 已在 npm deprecate**（2026-09-10 · 仅维护者可操作）；请钉 **`spec-wave@2.3.0`** 并按 `MIGRATION.md` 迁移。
...
````text
你 = 本仓库维护 Agent。把本仓从 @cyning/harness 迁到 spec-wave@2.3.0。

最小路径（必须，按序）：
1. package.json 的 devDependency：删除 @cyning/harness，改为 spec-wave（钉 2.3.0；曾用名 dsh-coding-kit）。
2. 在仓根执行：npx spec-wave upgrade --yes
   （读 .coding-kit/manifest.json 或 legacy .cyning-harness/manifest.json；写入 .coding-kit/manifest.json；version 钉 2.3.0，from_version 记旧号；不删除 .cyning-harness/；不覆盖 docs/tasks、reviews、invokes/by-task。）
...
**现行包**：**`spec-wave@2.3.0`** — **bump 已落 · 待发版**（tag/push/publish 仅人；当前已 published=**`2.2.1`**（`latest` · tag `v2.2.1`））。前一已发：**`2.2.0`**（闭环起步）· **`2.1.3`**（溯源自动化 patch）· **`2.1.2`**（改名收口）。
