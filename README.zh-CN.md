...
**dsh-coding-kit@1.12.1** 是 DeepSeek Harness（DSH）的 **bundle 插件**，并带 **P0 闸 CLI** 与 **G1–G7 过程命令**。纪律资产仍是 ICVO（Inform · Constrain · Verify · Orchestrate）。
...
两条入口同一 npm 包 **`dsh-coding-kit@1.12.1`**。插件面与 CLI 面互不替代。
...
钉 **dsh-coding-kit@1.12.1** 后可去掉 `@cyning/harness`。最小路径三步（必须，按序）：

1. 把 `devDependency` `@cyning/harness` 换成 `dsh-coding-kit`（钉 `1.12.1`）。
2. 在仓根执行 `npx dsh-coding-kit upgrade --yes`（读优先 `.coding-kit/manifest.json`，否则 legacy `.cyning-harness/manifest.json`；**写入** `.coding-kit/manifest.json`，`version` 钉 1.12.1，`from_version` 记旧号；**不删除** `.cyning-harness/`）。
...
Skill 安装为 **推荐、非必须**（最小路径不依赖 DSH 扫 skill）。命令一律 `npx dsh-coding-kit`。旧包 **`@cyning/harness` 已在 npm deprecate**（2026-09-10 · 仅维护者可操作）；请钉 **`dsh-coding-kit@1.12.1`** 并按 `MIGRATION.md` 迁移。
...
````text
你 = 本仓库维护 Agent。把本仓从 @cyning/harness 迁到 dsh-coding-kit@1.12.1。

最小路径（必须，按序）：
1. package.json 的 devDependency：删除 @cyning/harness，改为 dsh-coding-kit（钉 1.12.1）。
2. 在仓根执行：npx dsh-coding-kit upgrade --yes
   （读 .coding-kit/manifest.json 或 legacy .cyning-harness/manifest.json；写入 .coding-kit/manifest.json；version 钉 1.12.1，from_version 记旧号；不删除 .cyning-harness/；不覆盖 docs/tasks、reviews、invokes/by-task。）
...
**现行包（git）**：**`dsh-coding-kit@1.12.1`**（tag 就绪 · **npm `latest` 在人 publish 前仍为 `1.12.0`**）。前一发版：1.12.0 · 2026-09-10。
