...
describe('C* CLI P0 runtime', { concurrency: 1 }, () => {
  it('C-bin / version: package.json 为 1.11.0 且有 bin.dsh-coding-kit', async () => {
  ...
  it('R-HELP README: 完成态 1.11.0；双入口；加载≠注入；钉版后可去旧包', async () => {
    const readme = await readFile(path.join(KIT, 'README.md'), 'utf8')
    assert.match(readme, /dsh-coding-kit@1\.11\.0/)
  ...
  it('D8: bin 仅 dsh-coding-kit；patch 为 insert；pack 不含 SPEC.md', async () => {
    ...
    const info = parsed[0]
    assert.equal(info.version, '1.11.0')
    assert.match(String(info.filename ?? info.id ?? ''), /dsh-coding-kit-1\.11\.0/)
  ...
  it('C1: init --preset harness-only --yes 写出 version=1.11.0 且不写 S2', async () => {
    await withTemp(async (dir) => {
