...
describe('C* CLI P0 runtime', { concurrency: 1 }, () => {
  it('C-bin / version: package.json 为 2.3.1 且三 bin（spec-wave + specgate + dsh-coding-kit）', async () => {
  ...
  it('R-HELP README: 完成态 2.3.1；双入口；加载≠注入；钉版后可去旧包', async () => {
    const readme = await readFile(path.join(KIT, 'README.md'), 'utf8')
    assert.match(readme, /spec-wave@2\.3\.1/)
  ...
  it('D8: 三 bin spec-wave+specgate+dsh-coding-kit；patch 为 insert；pack 不含 SPEC.md', async () => {
    ...
    assert.match(String(info.filename ?? info.id ?? ''), /spec-wave-2\.3\.1/)
  ...
  it('C1: init --preset harness-only --yes 写出 version=2.3.1 且不写 S2', async () => {
    await withTemp(async (dir) => {
      const r = runCli([
        'init',
        '--preset',
