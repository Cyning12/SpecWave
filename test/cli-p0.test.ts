...
describe('C* CLI P0 runtime', { concurrency: 1 }, () => {
  it('C-bin / version: package.json 为 2.4.2 且三 bin（spec-wave + specgate + dsh-coding-kit）', async () => {
  ...
  it('R-HELP README: 完成态 2.4.2；双入口；加载≠注入；钉版后可去旧包', async () => {
  ...
  it('D8: 三 bin spec-wave+specgate+dsh-coding-kit；patch 为 insert；pack 不含 SPEC.md', async () => {
    ...
    const info = parsed[0]
    assert.equal(info.version, '2.4.2')
    assert.match(String(info.filename ?? info.id ?? ''), /spec-wave-2\.4\.2/)
