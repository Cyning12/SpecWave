...
describe('C* CLI P0 runtime', { concurrency: 1 }, () => {
  it('C-bin / version: package.json 为 2.1.0 且有 bin.dsh-coding-kit', async () => {
  ...
  it('R-HELP README: 完成态 2.1.0；双入口；加载≠注入；钉版后可去旧包', async () => {
  ...
  it('D8: bin 仅 dsh-coding-kit；patch 为 insert；pack 不含 SPEC.md', async () => {
    ...
    assert.match(String(info.filename ?? info.id ?? ''), /dsh-coding-kit-2\.1\.0/)
  ...
  it('C1: init --preset harness-only --yes 写出 version=2.1.0 且不写 S2', async () => {
    await withTemp(async (dir) => {
