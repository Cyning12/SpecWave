...
describe('C2 CLI upgrade compat', { concurrency: 1 }, () => {
  it('C2: from_version 单列 — 1.2.0 fixture upgrade --yes 钉 2.0.2 且 from_version=1.2.0', async () => {
  ...
  it('C2: 旧 manifest + S2 上 upgrade --yes 钉 2.0.2，from_version=旧号，S2 哈希不变', async () => {
    await withTemp(async (dir) => {
  ...
  it('check: 已钉 2.0.2 → 已是最新 exit 0；旧 version → 可升级', async () => {
    await withTemp(async (dir) => {
      // DEF-013：OLD_VERSION=2.24.0 数值上高于包版本 2.0.2，三向判定后属高版本分支；
