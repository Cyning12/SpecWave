...
describe('1.2.2 E2 adapters + README FAQ', { concurrency: 1 }, () => {
  ...
  it('P3-1: 根 README 钉 2.4.2；FAQ auto-install-peers=false；Prompt 围栏纪律', () => {
    const readme = readFileSync(README, 'utf8')
    assert.match(readme, /spec-wave@2\.4\.2/)
    ...
    assert.match(prompt, /version pinned at 2\.4\.2/)
  })

  it('D8: npm pack --dry-run 不含 SPEC.md；filename 含 2.4.2', () => {
    ...
    assert.ok(entry)
    assert.match(String(entry.filename ?? ''), /spec-wave-2\.4\.2/)
