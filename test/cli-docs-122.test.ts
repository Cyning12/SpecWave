...
describe('1.2.2 E2 adapters + README FAQ', { concurrency: 1 }, () => {
  ...
  it('P3-1: 根 README 钉 2.0.1；FAQ auto-install-peers=false；Prompt 围栏纪律', () => {
    ...
    assert.doesNotMatch(prompt, /~~~/)
    assert.match(prompt, /dsh-coding-kit@2\.0\.1/)
    assert.match(prompt, /version pinned at 2\.0\.1/)
  })

  it('D8: npm pack --dry-run 不含 SPEC.md；filename 含 2.0.1', () => {
    ...
    assert.ok(entry)
    assert.match(String(entry.filename ?? ''), /dsh-coding-kit-2\.0\.1/)
