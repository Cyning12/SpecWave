...
describe('D-DOC 1.2.1 README / renderReadme', { concurrency: 1 }, () => {
  it('D-DOC: README 命令表分列 skills install / build / check；钉 2.1.3 Prompt；路径对照；扫描验证（已对照上游源码）', () => {
    const readme = readFileSync(README, 'utf8')
    assert.match(readme, /npx spec-wave skills install/)
    assert.match(readme, /npx spec-wave skills build/)
    assert.match(readme, /npx spec-wave skills check/)
    ...
    assert.doesNotMatch(prompt, /~~~/)
    assert.match(prompt, /spec-wave@2\.1\.3/)
    assert.match(prompt, /Minimal path/)
    assert.match(prompt, /npx spec-wave upgrade --yes/)
    assert.match(prompt, /Commands are always npx spec-wave/)
    assert.match(prompt, /Never write npx @cyning\/harness skills build again/)
    assert.match(prompt, /Recommended \(not required/)
    assert.match(prompt, /npx spec-wave skills install/)
  ...
  it('D-DOC: README / assets/skills/README.md / renderReadme 无现行 harness skills build 入口', () => {
    ...
    assert.match(renderSrc, /npx spec-wave skills build/)
    assert.match(skillsReadme, /npx spec-wave skills build/)
    assert.match(skillsReadme, /npx spec-wave skills install/)
  ...
  it('D-DOC: generateSkills README 与 assets/skills/README.md 一致，且无 harness skills build 现行入口', () => {
    ...
    assert.match(generated, /npx spec-wave skills build/)
    assert.match(generated, /npx spec-wave skills build --with-execute-hats/)
  })
})
