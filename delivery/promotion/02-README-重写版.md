...
| Feature | 含义 |
|---------|------|
| **F1** | S2 过程域真源统一（消除 4 份硬编码） |
| **F2** | P0 门禁语义对齐（check/verify/gate-check/audit 机械判定） |
| **F3** | 旧产品线 `@cyning/harness` 迁移时间表 |
| **F4** | `.cyning-harness` 语义收敛（双重语义 → 单一定义） |
| **F5** | SPEC 版本钉一致性（SPEC 1.2.0 vs 包 1.10.0 治理） |

### 2.0（等待 1.x 完成）

- **F6**：宿主适配表——`AGENTS.md` 为默认落点 + 各宿主特化（Claude Code、Cursor、Copilot、Codex 等）
- DSH 由「唯一宿主」转为「**首个宿主**」
