...
**适用范围**：本文档覆盖 `dsh-coding-kit`（v1.10.0，`github.com/Cyning12/SpecWave`）从源码到消费者本机的**完整交付链路**——环境划分、资源清单、分发拓扑、CI/CD 流水线、制品与配置管理、发布策略、监控告警、应急回滚、上线前安全自检、容量与成本。不含业务边界与模块拆分（归《系统设计》），不含安全策略裁定（归《安全设计》）。
...
| R-S-002 | 发布件存储 | GitHub Releases | `Cyning12/SpecWave/releases/tag/vX.Y.Z` | gh-release | Release 附件 + 源码快照（GitHub 自动生成） | < 10 MB/版本 | prod | 复用 | GitHub 侧多副本 | GitHub 侧保留；git tag 同步（RELEASING ⑤） |
...
| R-Sec-006 | 代码所有权与合并管控 | CODEOWNERS + 分支保护规则 | 仓库 Settings（**是否已启用待人工确认**） | branch-guard | CI 未绿禁合（RELEASING ⑥）；`npm publish` 仅人；Agent 默认可 `npm version`/tag | int / prod | 待确认 | 不适用 | 防止未经门禁的变更进入 main 与发布通道 |
...
| 项 | 规范 / 值 | 说明 |
| --- | --- | --- |
| 代码仓库地址 | `https://github.com/Cyning12/SpecWave`（git+https） | 公共仓库，MIT；npm 包同名 `dsh-coding-kit` |
...
| 合并规范 | PR 合并进 `main`；**CI 未绿禁合**（RELEASING ⑥）；`npm publish` **仅人**（Agent 禁止 publish）；Agent 默认可 bump/tag；禁止从未提交工作树 publish（RELEASING ①，DEF-001 教训） | 合并即准入发布通道 |
...
| 制品仓库 | npm registry（包 `dsh-coding-kit`）+ GitHub Releases（`Cyning12/SpecWave`） | 双通道同源，registry 为权威分发源 |
...
- **常规迭代发布流程**：严格按 `RELEASING.md` 九步硬 checklist —— ① 工作树干净且全提交 → ② 四门全绿 → ③ CHANGELOG 版本节归拢 → ④ 版本钉同步（README 双文件 / 版本断言测试 / `ontology.yaml` / `discipline-coverage.yaml`）→ ⑤ `npm version` + tag（Agent 默认可做）→ ⑥ PR 合并 + CI 绿（未绿禁合）→ ⑦ `npm pack --dry-run` 核对 → ⑧ `npm publish`（仅人）→ ⑨ `npm view` 核验 + 过程档状态更新。可先行 `next` 通道试装（uat），观测窗 ≥ 24h 后再推 `latest`。
...
| 人工审批 | **至少 1 名 Owner**（⑧ `npm publish` 仅人；⑤ bump 可由 Agent；Agent/CI 禁 publish） | 生产发布前 |
...
- **Q3**：2026-09-09 起与 RELEASING 对齐 —— Agent 默认可 bump/tag；**仅** `npm publish` / `npm deprecate` 仅人；本文档同步更新，不再禁止 Agent version。
