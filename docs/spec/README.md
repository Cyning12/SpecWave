...
| `2.2.1`（patch 收尾行） | —（无独立 SPEC 夹） | **signed** · **待发版** · 属 `2_2-closed-loop-start` 验收后 patch（task `2-2-1-patch`） | 2.2.1 · patch：symlink realpath 归卡（C1 穿透封堵）+ pins fix 按文件聚合 + `.gitignore`/GLOSSARY 打包修复（验收报告 PASS-with-issues §4 #1/#2/#3/#6） |
