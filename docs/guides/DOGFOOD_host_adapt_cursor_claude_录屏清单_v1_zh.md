...
> **包钉**：建议 `npx dsh-coding-kit@2.1.0`（registry 未更新前用本仓源码或既有 `2.0.x`；叙事以本仓 README 2.1 为准）  
...
可选：先 `npx dsh-coding-kit@2.1.0 init --preset harness-only --yes`（过程根 `.coding-kit/`）；**host apply 不强制先 init**，但 init 利于后续 verify。
...
```bash
npx dsh-coding-kit@2.1.0 host validate
...
```bash
npx dsh-coding-kit@2.1.0 host apply --tools cursor,claude --profile core --json
# 或人类可读：
npx dsh-coding-kit@2.1.0 host apply --tools cursor,claude --profile core
```
...
```bash
npx dsh-coding-kit@2.1.0 host apply --tools cursor,claude --profile core --yes
...
```bash
cd "$DEMO"
npx dsh-coding-kit@2.1.0 verify --task docs/tasks/active/无.md || true
...
npx dsh-coding-kit@2.1.0 host apply --tools claude --profile core --yes
...
```bash
npx dsh-coding-kit@2.1.0 host apply --tools dsh --profile core --yes
...
npx dsh-coding-kit@2.1.0 host update --tools cursor --yes
# conflict：默认不覆盖
npx dsh-coding-kit@2.1.0 host update --tools cursor --yes --force
# --force 才覆盖（有备份目录可一闪）
