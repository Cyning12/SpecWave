...
license: MIT
compatibility: Requires npx spec-wave CLI（task lint / verify）· docs/tasks/active/ 目录约定
metadata:
  hat_id: "10-task"
  track: starter
---
...
- 产出 task 须 `npx spec-wave task lint --file <task>` PASS（v2.3+ · 结构闸 E1–E7 / W1+；v2.12+ 含 **W6** invoke 留档字段提醒）。
- 承接 **20-task-audit 审查**：按 `docs/harness/reviews/*_audit_*.md` 回填 task。
