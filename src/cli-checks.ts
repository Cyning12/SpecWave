...
// 否定守卫：退回（前置 无需/不/未 除外）· 否定+通过（不/未 与 通过 之间允许 ≤3 任意字符 · 2.4.1 NEW-1
// 放宽：封堵「不予通过」「不 通过」插字断链绕过面）· no pass / reject（英文基本形态 · i 旗标）·
// 内容阻塞（前置 零 除外）——命中即不通过。
// 已知残余（R1 审查 §3-1 裁决留痕）：`.` 不跨行，「不\n通过」换行形态仍漏网（攻击者须主动换行 ·
// 与 NEW-11 广义词表面同属 3.0 语义化议题）。
...
const REVIEW_NEG_RE = /((?<!无需)(?<!不)(?<!未)退回|不.{0,3}通过|未.{0,3}通过|no\s*pass|reject|(?<!零)内容阻塞)/i
