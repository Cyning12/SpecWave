...
> **⚠️ `dsh-coding-kit` 已 deprecate** —— 请**直接**安装正式包：`npm i spec-wave@2.3.1`（patch · 2.3.0 验收三项修复（发布卫生/闸语义）；**tag/npm 同 commit 发版**后以 registry `latest` 为准）。**勿再**把废弃中间包当作迁移终点。  
> **现行包**：**`spec-wave@2.3.1`**（正式包名 / 正式 bin；曾用名 `dsh-coding-kit`）  
...
> **包钉**：请钉 `spec-wave@2.3.1`（本文件不代替 `package.json`）  
...
1. **依赖**：`package.json` 将 `dsh-coding-kit`（或 `@cyning/harness`）改为 **`spec-wave`**（钉 `2.3.1`；CHANGELOG 见 `[2.3.1]` 节）。  
...
> **链式风险**：上列 harness 文案仍指向已 deprecate 的 `dsh-coding-kit`。**请忽略该钉点**，直接 `npm i spec-wave@2.3.1`。registry 改文案仅人（`HG-DEPRECATE-HARNESS`）。
